...
class AdminController(BaseController):
    ...
    def add_category(self, nome: str, descricao: str) -> Dict[str, Any]:
        if not nome:
            return self._error_response("Nome é obrigatório")
        try:
            existente = self.category_repo.buscar_por_nome(nome)
            if existente:
                return self._error_response(f"Categoria '{nome}' já existe.")
    ...
    def update_category(
        self, categoria_id: int, nome: str, descricao: str
    ) -> Dict[str, Any]:
        try:
            existente = self.category_repo.buscar_por_nome(nome)
            if existente and existente["id"] != categoria_id:
                return self._error_response(
                    f"Já existe outra categoria com o nome '{nome}'."
                )
            dados = {
                "id": categoria_id,
                "nome": nome,
    ...
    def toggle_category(
        self, categoria_id: int, novo_status_bool: bool
    ) -> Dict[str, Any]:
        try:
    ...
    def get_order_details(self, pedido_id: int) -> Dict[str, Any]:
        try:
            ...
            return self._success_response("Detalhes", pedido)
        except Exception as e:
            return self._error_response("Erro", e)
